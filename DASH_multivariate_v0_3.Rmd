...
df_sim_VAR <- eventReactive(input$buttSimVAR, {
  ...
  ### trend matrix to add
  mTrend <- matrix(1:input$modHorizonByInputPeriod, ncol = 1) %*% matrix(vTrend, nrow=1)
  res <- matrix(NA, nrow = input$nrSimVAR, ncol = input$modHorizonByInputPeriod*dim(mEst)[1])
  for (idx in 1:input$nrSimVAR){
    res[idx, ] <- unlist(c(Sim_VAR_N_Period(w=vCont, A=mEst, C=mCovMat, N=input$modHorizonByInputPeriod) + mTrend ))
  }
  res <- data.frame(res, stringsAsFactors = F)
  names(res) <- unlist(purrr::map(names(tmpEst$df.est), function(x) paste0(x, "_Period", 1:input$modHorizonByInputPeriod)))
  res
})

df_sim_VAR_delta <- reactive({
  diff(df_sim_VAR())
})

# df_sim_VAR_forecast <- reactive{(
#   
# )}
```
...
fillRow(width="100%", flex = c(NA, 1), 
  wellPanel(
    fluidRow(
      ...
      sliderInput(inputId = "modHorizonByInputPeriod", label = "Nr of periods ahead to Sim", min = 1, max = 24, value = 8),
      sliderInput(inputId = "nrSimVAR", label = "Nr of simulation", min = 100, max = 30e3, value = 500),
      sliderInput(inputId = "nrBinHistVAR", label = "Nr of bins for histogram", min = 1, max = 30, value = 10),
      sliderInput(inputId = "period2ShowHist", label = "n-th period to show in histgram", min = 1, max = 24, value = 6), 
  ...
  fluidRow(  
    column(12, renderUI({
      ...
      renderNumInputs(as.numeric(tmp), paste0(names(tmp), "_foreT0"), names(tmp), "Forecast t0")
      })
    )
  )
)
