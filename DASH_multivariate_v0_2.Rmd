...
df_sim_VAR <- reactive({
  ...
  if (input$selOverrideVAR == "Original.Estiamte"){
    vCont <- as.numeric(tmpEst$df.est[row.names(tmpEst$df.est.pVal) == "const", ])
    vTrend <- as.numeric(tmpEst$df.est[row.names(tmpEst$df.est.pVal) == "trend", ])
    mEst <- t(as.matrix(tmpEst$df.est[!(row.names(tmpEst$df.est.pVal) %in% c("const", "trend")), ]))
    ...
    varNameCoeff <- unlist(purrr::map(1:(length(varName.initial)-2), function(x) paste0(paste0(varName.initial[x], ".coeff."), 1:nVarinY)))
    ## get mEst
  ...
  mAr::mAr.sim(w=vCont, A=mEst, C=mCovMat, N=input$nrSimVAR) + 
    matrix(1:input$nrSimVAR, ncol = 1) %*% matrix(vTrend, nrow=1)
})
...
renderUI({
  ...
  fillRow(
    wellPanel(
      fluidRow(
        column(width=2, selectInput("selOverrideVAR", label="VAR parameters to use", choices = c("Original.Estiamte", "Overriden"))),
        column(width=3, sliderInput(inputId = "nrSimVAR", label = "Nr of simulation", min = 1000, max = round(100/nVarinY)*1e3, value = 10e3)),
        column(width=3, sliderInput(inputId = "nrBinHistVAR", label = "Nr of bins for histogram", min = 1, max = 100, value = 30))
      )
    )
  )
})
...
renderPrint({
  summary(VAR(df_sim_VAR(), p=input$sel.VAR.lag, type="both"))
})
```
