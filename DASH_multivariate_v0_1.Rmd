...
fillRow(flex = c(1, NA), 
  renderUI({
    tmp <- lst.modEst.VAR.select()
    vec.def.est <- round(tmp$df.est[, 1], 2)
    vec.def.est[tmp$df.est.pVal[, 1] > 0.05] <- 0.0
    vec.def.name <- row.names(tmp$df.est.pVal)
    lapply(1:length(vec.def.name), function(idx) {
      numericInput(vec.def.name[idx], label=vec.def.name[idx], value=vec.def.est[idx])
    })
})
)
